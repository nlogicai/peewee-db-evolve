...
if PW3:
    ...
    def drop_foreign_key(db, migrator, table_name, fk_name):
        ...
        migration = migrator._alter_table(migrator.make_context(), table_name).literal(drop_stmt).sql(
            pw.Entity(fk_name))
        return extract_query_from_migration(migration)
    ...
    def rename_column(db, migrator, table, ocn, ncn, field):
        if is_mysql(db):
            ...
            migration = migrator._alter_table(ctx, table).literal(' CHANGE ').sql(pw.Entity(ocn)).literal(' ').sql(
                field.ddl(ctx))
        else:
    ...
    def change_column_type(db, migrator, table_name, column_name, field):
        ...
        ctx = migrator.make_context()
        if is_postgres(db):
            migration = migrator._alter_column(ctx, table_name, column_name).literal(' TYPE ').sql(
                field.ddl_datatype(ctx))
        elif is_mysql(db):
    ...
    def drop_default(db, migrator, table_name, column_name, field):
        op = pw.Clause(pw.SQL('ALTER TABLE'), pw.Entity(table_name), pw.SQL('ALTER COLUMN'), pw.Entity(column_name),
                       pw.SQL('DROP DEFAULT'))
        return normalize_whatever_junk_peewee_migrations_gives_you(migrator, op)
    ...
    def rename_column(db, migrator, ntn, ocn, ncn, field):
        ...
        if is_mysql(db):
            junk = pw.Clause(
                pw.SQL('ALTER TABLE'), pw.Entity(ntn), pw.SQL('CHANGE'), pw.Entity(ocn),
    ...
    def change_column_type(db, migrator, table_name, column_name, field):
        column_type = _field_type(field)
        if is_postgres(db):
            op = pw.Clause(pw.SQL('ALTER TABLE'), pw.Entity(table_name), pw.SQL('ALTER'), field.as_entity(),
                           pw.SQL('TYPE'), field.__ddl_column__(column_type))
        elif is_mysql(db):
            op = pw.Clause(
                *[pw.SQL('ALTER TABLE'), pw.Entity(table_name), pw.SQL('MODIFY')] + field.__ddl__(column_type))
        else:
            raise Exception('how do i change a column type for %s?' % db)
        return normalize_whatever_junk_peewee_migrations_gives_you(migrator, op)
    ...
    def add_not_null(db, migrator, table, column_name, field):
        ...
        if field.default is not None:
            ...
            op = pw.Clause(pw.SQL('UPDATE'), pw.Entity(table), pw.SQL('SET'), field.as_entity(), pw.SQL('='), default,
                           pw.SQL('WHERE'), field.as_entity(), pw.SQL('IS NULL'))
    ...
    def indexes_on_model(model):
        return [pw.IndexMetadata('', '', [_column_name(f)], f.unique, _table_name(model)) for f in
                model._fields_to_index()]
...
def auto_detect_migrator(db):
    ...
    raise Exception("could not auto-detect migrator for %s - please provide one via the migrator kwarg" % repr(
...
ForeignKeyMetadata = collections.namedtuple('ForeignKeyMetadata',
                                            ('column', 'dest_table', 'dest_column', 'table', 'name'))
...
def calc_column_changes(db, migrator, ntn, existing_columns, defined_fields, existing_fks_by_column):
    ...
    for col_name in not_new_columns:
        ...
        if column_def_changed(db, existing_col, defined_col):
            ...
            should_cast = different_type and can_convert(existing_col.data_type)
            ...
            if DIFF_DEFAULTS:
                if normalize_default(existing_col.default) is not None and normalize_default(
                        defined_col.default) is None:
            ...
            if not (len_alter_statements < len(alter_statements)):
                if existing_col.data_type == u'array':
                    ...
                    raise Exception(
                        "In table %s I don't know how to change %s into %s" % (repr(ntn), existing_col, defined_col))
...
def calc_changes(db, ignore_tables=None):
    ...
    for etn, ecols in existing_columns_by_table.items():
        ...
        adds, deletes, renames, alter_statements = calc_column_changes(
            db, migrator, ntn, ecols, defined_fields, existing_fks_by_column
        )
    ...
    for ntn, model in table_names_to_models.items():
        ...
        existing_indexes_for_table = [
            i
            for i in existing_indexes.get(etn, [])
            if not any([(c in deletes) for c in i.columns])
        ]
        to_run += calc_index_changes(migrator, existing_indexes_for_table, model)
...
def calc_index_changes(migrator, existing_indexes, model):
    to_run = []
...
def evolve(db, interactive=True, ignore_tables=None):
    if interactive:
        print((colorama.Style.BRIGHT + colorama.Fore.RED + 'Making updates to database: {}'.format(
            db.database) + colorama.Style.RESET_ALL))
...
def _execute(db, to_run, interactive=True, commit=True):
    ...
    try:
        ...
        print('------------------------------------------')
        print(
            colorama.Style.BRIGHT + colorama.Fore.RED + ' SQL EXCEPTION - ROLLING BACK ALL CHANGES' + colorama.Style.RESET_ALL)
        print('------------------------------------------')
        print()
        raise e
...
def _confirm(db, to_run):
    ...
    print()
    while True:
        print('Do you want to run %s? (%s)' % (('these commands' if len(to_run) > 1 else 'this command'),
                                               ('type yes, no or test' if is_postgres(db) else 'yes or no')), end=' ')
